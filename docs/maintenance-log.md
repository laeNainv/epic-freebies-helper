...
### 2026-09-13 修正 GLM 候选拖拽题的动作数量与坐标结构提示

- 现象：Actions run `34514744011` 在密码登录阶段反复收到 `image_drag_multi`；一次 GLM 返回的 `paths` 把 `start_point`、`end_point` 写成标量，解析器明确拒绝，另一次请求在 50 秒预算内超时。工件截图显示题目实际是多个候选部件对应一个空位，并非所有候选都要拖动。
- 根因判断：GLM 拖拽提示虽然要求输出 `paths`，但没有给出嵌套 `PointCoordinate` 的完整 JSON 结构；同时没有明确区分“多个候选”与“多个必做动作”，模型可能生成错误的字段形状或多余路径。保留 GLM 4.6 thinking，因为此前工件重放已确认全局关闭会降低坐标准确率。
- 改动文件：`app/extensions/llm_adapter.py`、`docs/maintenance-log.md`。
- 处理结果：拖拽提示加入精确 JSON 模板，强制起点和终点使用含整数 `x/y` 的对象；单空位候选题只返回匹配候选的一条路径，多动作题仍按实际动作数输出。按仓库规则不执行测试；仅执行 Python 编译和 diff 检查。真实 hCaptcha 通过率仍需以后一次人工触发的 Actions 运行确认。
